...
const { contentTypes } = require('siapi-utils');
...
const buildShadowCrud = ctx => {
  const models = Object.values(siapi.contentTypes).filter(model => model.plugin !== 'admin');
  const components = Object.values(siapi.components);
    ...
    .forEach(attribute => {
      ...
      if (components.length === 0) {
        ...
        const componentsTypeNames = components.map(componentUID => {
          const compo = siapi.components[componentUID];
          if (!compo) {
            throw new Error(
      ...
      schema.resolvers[typeName] = {
        __resolveType(obj) {
          return siapi.components[obj.__component].globalId;
        },
      };
...
const buildAssocResolvers = model => {
  ...
  return associations
    ...
    .reduce((resolver, association) => {
      ...
      const targetModel = siapi.getModel(target, association.plugin);
      ...
      switch (nature) {
        ...
        case 'manyToManyMorph': {
          resolver[alias] = async obj => {
            ...
            const entry = await siapi.query(model.uid).findOne(params, [alias]);
        ...
        default: {
          resolver[alias] = async (obj, options) => {
            ...
            const loader = siapi.plugins.graphql.services['data-loaders'].loaders[targetModel.uid];
