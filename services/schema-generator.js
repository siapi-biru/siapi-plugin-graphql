...
const generateSchema = () => {
  const isFederated = _.get(siapi.plugins.graphql.config, 'federation', false);
  const shadowCRUDEnabled = siapi.plugins.graphql.config.shadowCRUD !== false;

  const _schema = siapi.plugins.graphql.config._schema.graphql;
...
const writeGenerateSchema = schema => {
  ...
  return siapi.fs.writeAppFile('exports/graphql/schema.graphql', printSchema);
};
