...
const { convertRestQueryParams, buildQuery } = require('siapi-utils');
  ...
  let resolvers = {
    [aggregatorGlobalId]: {
      count(obj) {
        ...
        if (opts._q) {
          // allow search param
          return siapi.query(modelName, model.plugin).countSearch(opts);
        }
        return siapi.query(modelName, model.plugin).count(opts);
      },
      totalCount() {
        return siapi.query(modelName, model.plugin).count({});
      },
    },
  };
...
const formatModelConnectionsGQL = function({ fields, model: contentType, name, resolver }) {
  ...
  const model = siapi.getModel(contentType.uid);
