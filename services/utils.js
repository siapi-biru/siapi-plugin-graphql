...
const { QUERY_OPERATORS } = require('siapi-utils');
...
/**
 * Merges siapi graphql schema together
...
const amountLimiting = (params = {}) => {
  const { amountLimit } = siapi.plugins.graphql.config;
...
const getActionFn = details => {
  ...
  if (plugin) {
    return _.get(siapi.plugins, [_.toLower(plugin), 'controllers', _.toLower(controller), action]);
  }

  return _.get(siapi.api, [_.toLower(api), 'controllers', _.toLower(controller), action]);
};
