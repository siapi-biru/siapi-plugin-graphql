...
module.exports = {
  ...
  initializeLoader() {
    ...
    Object.values(siapi.contentTypes).forEach(model => this.createLoader(model.uid));
  },
  ...
  async makeQuery(modelUID, query = {}) {
    if (query.single === true) {
      return siapi.query(modelUID).findOne(query.filters, []);
    }

    return siapi.query(modelUID).find(query.filters, []);
  },
};
